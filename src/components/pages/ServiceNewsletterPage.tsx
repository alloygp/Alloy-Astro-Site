...
// ---------------------------------------------------------------------------
// Newsletter mockup — branded vs. manager-made, used in hero
// ---------------------------------------------------------------------------
function NewsletterMockup({ variant = 'branded' }: { variant?: 'branded' | 'generic' }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', boxShadow: 'inset 0 0 0 1px rgba(0,0,0,0.04)' }}></div>
      ...
      <div style={{ padding: '20px 24px 24px', display: 'grid', gridTemplateColumns: isBranded ? '1.4fr 1fr' : '1fr', gap: 16 }}>
        <div>
          ...
          {([1, 2, 3, 4] as const).map(i => (
            <div key={i} style={{ height: 5, background: '#e6e0d4', borderRadius: 1, marginBottom: 5, width: i === 4 ? '62%' : '100%', opacity: 0.65 }}></div>
          ))}
        ...
        {isBranded && (
          <div style={{ background: 'linear-gradient(160deg, #fbe2eb 0%, #fbf2d6 100%)', borderRadius: 4, padding: 10, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 100 }}>
            ...
            <div>
              <div style={{ width: 28, height: 28, borderRadius: 999, background: '#fff', marginBottom: 6, opacity: 0.7 }}></div>
...
// ---------------------------------------------------------------------------
// Editorial calendar — 12-month rhythm strip
// ---------------------------------------------------------------------------
      ...
      {months.map((mo, i) => (
        <div key={mo.m} style={{ padding: '16px 8px', borderTop: `3px solid ${mo.c}`, background: i % 2 === 0 ? '#fafaff' : 'transparent', borderRadius: 4, minHeight: 120, display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 3 }}>
            <div style={{ height: 3, background: mo.c, borderRadius: 1, opacity: 0.95 }}></div>
            <div style={{ height: 3, background: mo.c, borderRadius: 1, opacity: 0.55, width: '70%' }}></div>
            <div style={{ height: 3, background: mo.c, borderRadius: 1, opacity: 0.30, width: '40%' }}></div>
          </div>
        </div>
      ))}
    </div>
...
// FAQ accordion item
// ---------------------------------------------------------------------------
interface FAQItemProps { q: string; a: string; accent?: string; }

function FAQItem({ q, a, accent = PINK }: FAQItemProps) {
  ...
  return (
    <div style={{ borderTop: '1px solid var(--border-subtle)' }}>
      ...
      {open && (
        <div style={{ padding: '0 28px 24px', fontSize: 16, color: '#555', lineHeight: 1.7 }}>{a}</div>
      )}
    </div>
  );
}
...
const AUDIENCES = [
  {
    tag: 'Board members',
    tagColor: BLUE,
    eyebrow: 'Looking for a template?',
    headline: 'We don\'t sell templates — and your management company shouldn\'t either.',
    body: 'If you\'re a director Googling "HOA newsletter template," you\'re patching a gap your management company should be filling. Forward this page to them. The right CAM firm produces your community newsletter as part of the service — branded, scheduled, and read.',
    cta: 'Send this to your manager',
    ctaHref: 'mailto:?subject=Newsletter%20production%20by%20our%20management%20company&body=Came%20across%20this%20—%20curious%20if%20we%20could%20do%20this%20instead%20of%20building%20one%20each%20month%3A%20https%3A%2F%2Falloygp.co%2Fservices%2Fhoa-newsletter-production%2F',
  },
  {
    tag: 'CAM operators',
    tagColor: PINK,
    eyebrow: 'Producing newsletters in-house?',
    headline: 'Newsletters are the cheapest retention asset you\'re not making.',
    body: 'A newsletter that actually arrives, looks professional, and answers the questions boards keep asking is worth more than any sales deck. We produce them for your portfolio at scale — same masthead, customized per association, on a schedule your managers don\'t have to manage.',
    cta: 'Talk to Alloy',
    ctaHref: '/strategic-review-request',
  },
];
...
const PROCESS = [
  { num: '01', h: 'Portfolio audit', d: 'We inventory every association in your portfolio — name, brand assets, board composition, current comms cadence (or lack of it).', color: PINK },
  { num: '02', h: 'Editorial system build', d: '12-month theme calendar, masthead system, voice guide. Approved by your leadership before issue one.', color: YELLOW },
  { num: '03', h: 'Per-issue production', d: 'Draft → manager review (24-hour turn) → design → optional compliance pass → distribution.', color: GREEN },
  { num: '04', h: 'Reporting & iteration', d: 'Monthly engagement readout. We trim what boards skip and double down on what they read.', color: BLUE },
];
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 64, alignItems: 'center' }}>
            <div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
                <span style={{ width: 8, height: 8, borderRadius: 999, background: GREEN, boxShadow: `0 0 0 4px rgba(174,215,208,0.18)` }}></span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 24, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.10)', flexWrap: 'wrap' }}>
                {([
                  { k: 'Issues / yr', v: '12' },
                  { k: 'Mgr time / issue', v: '≈ 0 hrs' },
                  { k: 'Inside BoardSuite', v: 'Yes' },
                ] as const).map(s => (
                  <div key={s.k}>
            ...
            <div style={{ position: 'relative', height: 540, display: 'grid', placeItems: 'center', overflow: 'visible' }}>
              <div style={{ position: 'absolute', inset: '8% 4% 8% 4%', background: 'radial-gradient(ellipse at center, rgba(245,216,128,0.25) 0%, transparent 70%)', filter: 'blur(20px)' }}></div>
      ...
      <section className="section section-white" style={{ paddingTop: 72, paddingBottom: 72 }}>
        <div className="container">
          <div style={{ marginBottom: 36, maxWidth: 760 }}>
            ...
            <h2 className="display-lg" style={{ margin: '14px 0 10px', color: PURPLE }}>
              If you searched <span style={{ fontStyle: 'italic', color: PINK }}>"HOA newsletter,"</span><br />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            {AUDIENCES.map(a => (
              <div key={a.tag} className="card card-pad" style={{ borderTop: `5px solid ${a.tagColor}`, display: 'flex', flexDirection: 'column', gap: 14, padding: 36 }}>
                <div><Tag color={a.tagColor === PINK ? 'pink' : 'blue' as any}>{a.tag}</Tag></div>
        ...
        <div className="container">
          <div style={{ marginBottom: 36, display: 'grid', gridTemplateColumns: '1fr auto', gap: 48, alignItems: 'end' }}>
            <div>
              ...
              <h2 className="display-lg" style={{ margin: '14px 0 10px', color: PURPLE }}>
                A 12-month editorial rhythm — built around <span style={{ color: PINK }}>your portfolio\'s actual year.</span>
              </h2>
            ...
            <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end', maxWidth: 240 }}>
              {[
                { color: PINK, label: 'Financial' },
                { color: GREEN, label: 'Community' },
                { color: BLUE, label: 'Governance' },
                { color: YELLOW, label: 'Seasonal' },
              ].map(leg => (
                <div key={leg.label} style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <span style={{ width: 10, height: 10, background: leg.color, borderRadius: 2 }}></span>
                  <span style={{ fontSize: 12, color: '#666' }}>{leg.label}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="container">
          <div style={{ marginBottom: 40, maxWidth: 760 }}>
            ...
            <h2 className="display-lg" style={{ margin: '14px 0 0', color: '#fff' }}>
              From zero to first issue in <span style={{ color: YELLOW }}>30 days.</span>
            </h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24 }}>
            {PROCESS.map(s => (
              <div key={s.num} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.10)', borderTop: `3px solid ${s.color}`, borderRadius: 12, padding: 28 }}>
                <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, color: s.color, fontSize: 12, letterSpacing: '0.16em', marginBottom: 12 }}>{s.num}</div>
      ...
      <section className="section section-white">
        <div className="container-narrow">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, borderTop: '1px solid var(--border-subtle)' }}>
            {FAQS.map((f, i) => <FAQItem key={i} q={f.q} a={f.a} accent={PINK} />)}
          </div>
        </div>
      </section>
